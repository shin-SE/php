...
if(isset($_POST['user_name'])&&isset($_POST['gender'])){
	//データの受け取る
	// $email = '0123456789abcdefghij@gmail.com';
	...
	$namelegal = namelegal($user_name);
	$passwordlegal = passwordlegal($_POST['password']);
	
	if ($is_comp[0] == true) {
		if ($passwordlegal[0] == true) {
			if($namelegal[0] == true) {
				//日本東京タイムゾーン指定
			...
			} else if (ctype_space($user_name)) {
				echo "<script type='text/javascript'>alert('空白文字だけでは登録できません。');</script>";
			} else {
				echo $namelegal[1];
			}
		} else {
			echo $passwordlegal[1];
		}
	} else {
		echo $is_comp[1];
	}
...
<section>
	<form action="" method="post">
		...
		<br><br>
		<!-- パスワード一回目の入力 -->
		<label>パスワードを入力してください。数字、小文字、大文字含めて8~16文字</label><br><br>
